    ...
    return (
        <div className='grid grid-cols-4  gap-5 '>
            {products.map(product => {
                return (
                    <ProductCard key={product.id} id={product.id} title={product.title} amount={product.price} image={product.image} category={product.category} addToCart={() => addToCart(product)} isInCart={isInCart(product.id)} quantity={isInCart(product.id) ? getQuantity(product.id) : 0} handleQuantity={handleQuantity} />
                )
            })}
        </div>
    )
